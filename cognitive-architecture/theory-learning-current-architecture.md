...
이 시스템은 **같은 LLM을 쓰면서도, 이전 시도에서 얻은 관측·이론·다음 실험 계획을 다음 입력에 전달하는 것**을 목표로 합니다. 모델 가중치를 학습하거나 하네스 코드를 스스로 수정하지는 않습니다. 현재 문장 실험에서는 사용자가 웹에서 시작하면 Python 서버가 생성·채점·학습 묶음을 이어서 진행합니다.
...
세 역할은 작업 목적과 적용 범위를 공유합니다. 공통 목적·시도 기억의 자동 전달은 현재 점수 실험에 연결되어 있으며, 다른 프로젝트에도 해당 내용을 전달하는 연결이 필요합니다. 실제 제공 자료는 역할과 실행에 따라 다르며, 아래의 “근거 읽기”는 허용된 기록만 읽는 권한입니다.
...
| **Integrator — 통합안 작성자** | 추출 내용, 검토, 실제 조사 결과, 기존 KB, 허용된 근거 | 근거를 종합해 이론·관계 변경을 제안합니다. 결론이 안 나도 해본 것·불확실성·다음 비교안을 남깁니다. | 같은 읽기 도구. KB 저장·실험 실행 권한은 없습니다. 형식·출처 오류를 고치는 재요청에는 도구가 없습니다. |
...
- **유용한 이론을 못 찾은 경우:** KB 변경이 없을 수 있습니다. 현재 점수 실험에서는 그래도 시도 기억과 다음 비교안을 남깁니다.
...
학습 역할은 **Extractor·Reviewer·Integrator의 세 가지**입니다. 여기에 과제를 수행하는 **Worker**, 사용자가 번역을 요청할 때 사용하는 **Translator**가 있습니다. “역할 세 가지”는 항상 프로세스나 모델 호출이 정확히 세 개라는 뜻은 아닙니다. 결과 재검토나 잘못된 JSON 수정에는 추가 호출이 생길 수 있습니다.
...
|---|---|---|---|
| **Worker — 과제 수행** | 원래 과제, 공통 목적, 고정된 KB 문맥, 이전 시도 기억, 이번 제출에 배정된 비교안 | 문장 실험에서는 `search_theories`, `read_theory`, `read_evidence`만 제공합니다. 별도 일반 Worker 실행은 설정에 따라 Python 도구를 사용할 수 있습니다. | 실제 답안, 사용했다고 보고한 이론 수정본 ID. 점수는 이후 환경이 매깁니다. |
...
이 선택은 어휘·태그 기반의 결정적 휴리스틱입니다. 논리적으로 동치인지 판정하여 KB 이론을 병합하거나 삭제하는 기능은 아닙니다. `context_tokens`의 기본값은 16,000이며 설정으로 바꿀 수 있습니다. 현재는 UTF-8 바이트 수로 보수적으로 적용되는 **이론 문맥의 크기 한도**입니다. 공통 목적·시도 기억·출력 스키마 등을 합친 모델 요청 전체 크기와는 다릅니다.
...
먼저 왼쪽 위 **작업공간**을 확인한 다음, 왼쪽 **문장 실험**으로 들어가시면 반복 실행과 학습 전후 비교를 함께 볼 수 있습니다.
